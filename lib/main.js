...
app.get('/search/:indexName?', function (req, res) {
	...
	if (!indexName){
        ...
        var cacheResult;
        if(cacheResult = cache.get(hashKey)){   //if we have a hit,
            ...
            cache.addItem(hashKey,cacheResult.value);    //adding query and result as our most recent hit
        }
        else{   //accessing db
            ...
            app.get('indexer').getTerms(query).then(function(queryTerms){
                ...
                console.log(docs)
                cache.addItem(hashKey,docs);    //adding query and result as our most recent hit
            });
        }
	}
});
